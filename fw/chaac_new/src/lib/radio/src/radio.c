 ...
 */
void RadioOnTxTimeoutIrq( TimerHandle_t xTimer );

/*!
 * \brief Rx timeout timer callback
 */
void RadioOnRxTimeoutIrq( TimerHandle_t xTimer );
...
StaticTimer_t xTxTimeoutTimerBuffer;
StaticTimer_t xRxTimeoutTimerBuffer;
...
void RadioInit( RadioEvents_t *events )
{
    ...
    SX126xSetDioIrqParams( IRQ_RADIO_ALL, IRQ_RADIO_ALL, IRQ_RADIO_ALL, IRQ_RADIO_NONE );

    // Initialize driver timeout timers
    // TimerInit( TxTimeoutTimer, RadioOnTxTimeoutIrq );
    // TimerInit( RxTimeoutTimer, RadioOnRxTimeoutIrq );
    TxTimeoutTimer = xTimerCreateStatic(
        "RadioTxTimer",             // Name
        100,                        // Period
        pdFALSE,                    // auto-reload
        NULL,                       // ID
        RadioOnTxTimeoutIrq,        // Callback
        &xTxTimeoutTimerBuffer);    // Static buffer
    RxTimeoutTimer = xTimerCreateStatic(
        "RadioRxTimer",
        100,
        pdFALSE,
        NULL,
        RadioOnRxTimeoutIrq,
        &xRxTimeoutTimerBuffer);
...
bool RadioIsChannelFree( uint32_t freq, uint32_t rxBandwidth, int16_t rssiThresh, uint32_t maxCarrierSenseTime )
{
    ...
    while( (xTaskGetTickCount() - carrierSenseTime) < maxCarrierSenseTime )
    {
...
void RadioSend( uint8_t *buffer, uint8_t size )
{
    ...
    xTimerChangePeriod( TxTimeoutTimer, pdMS_TO_TICKS(TxTimeout), 10 );
...
void RadioRx( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
        xTimerChangePeriod( RxTimeoutTimer, pdMS_TO_TICKS(timeout), 10 );
...
void RadioSetTxContinuousWave( uint32_t freq, int8_t power, uint16_t time )
{
    ...
    xTimerChangePeriod( TxTimeoutTimer, pdMS_TO_TICKS(timeout), 10 );
    xTimerStart( TxTimeoutTimer, 0 );
}
...
extern TaskHandle_t pxRadioIrqTaskHandle;
void RadioOnDioIrq( void* context )
{
    (void)context;
    IrqFired = true;
    if(pxRadioIrqTaskHandle != NULL) {
        vTaskNotifyGiveFromISR(pxRadioIrqTaskHandle, NULL);
    }
}
