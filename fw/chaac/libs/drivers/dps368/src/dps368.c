...
int16_t dps368_measure_temp_once(float *result) {
    ...
    os_time_ms_to_ticks(
            (calcBusyTime(0U, m_tempOsr) / DPS__BUSYTIME_SCALING) + 
                DPS310__BUSYTIME_FAILSAFE, &delay_ticks);
    if (delay_ticks < 2) {
        delay_ticks = 2;
    }
...
static int16_t readByte(uint8_t regAddress) {
    ...
    do {
        ...
        rval = hal_i2c_master_write(0, &i2c_data, 10, 0);
        if (rval != 0) {
            break;
        }
